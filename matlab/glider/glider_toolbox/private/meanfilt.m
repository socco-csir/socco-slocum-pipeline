...
% Running-Mean Filter
%
% GEOMAR SVN $Id: meanfilt.m 668 2020-02-11 10:19:35Z gkrahmann@geomar.de $
% 
% function [y,stdy] = meanfilt(x,k,[dim],[method])
%
% Running-Mean Filter
...
%           method  ['const'] - method may be 'const' , 'triang',
%                               'const2' or 'triang-onesided'
%
...
%
% version 2  last change 28.05.2014
...
% added triang-onesided filter                  GK, 28.05.2014  0.1.3-->2
...
if nargin<3
  ...
  if isempty(dim)
    dim=1;
  elseif dim>ndims(x)
    error('wanted dimension does not exist')
    return
  end
end
...
if (2*k>m)
  warning('The filter-width is too large !!')
...
elseif strcmp(method,'triang')
  ...
  stdy = [];
elseif strcmp(method,'triang-onesided')
  tri=[1:k]'*ones(1,size(xn,2));
  for i=1:m
    y(i,:) = meanweight(xn(k+[i-k+1:i],:),tri,1,0) ;
  end
  stdy = [];
else
